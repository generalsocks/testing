...
export class AppComponent implements OnInit, OnDestroy {
  ...
  DateSA() {
    const now = new Date();
    const formattedDate = `${now.getUTCFullYear()}-${(now.getUTCMonth() + 1).toString().padStart(2, '0')}-${now.getUTCDate().toString().padStart(2, '0')} ${now.getUTCHours().toString().padStart(2, '0')}:${now.getUTCMinutes().toString().padStart(2, '0')}:${now.getUTCSeconds().toString().padStart(2, '0')}`;
  ...
  populateClinician(clinicians: any[]) {
    ...
    if (selectElement != null) {selectElement.innerHTML = '';}
    

    // Iterate through the cityArray and create <option> elements
    clinicianArray.forEach((clinicianRecord: { id: { toString: () => string; }; cname: string | null; }) => {
